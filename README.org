...
**** Service
#+BEGIN_SRC typescript
class Service<Params> implements ImplementationOf<IAPI> {
  constructor(
...
#+BEGIN_SRC typescript
function createRPCServerOnExtraWebSocket(
  service: ImplementationOf<IAPI>
, ws: ExtraWebSocket
): () => void
#+END_SRC

**** createRPCServerOnWebSocket
#+BEGIN_SRC typescript
function createRPCServerOnWebSocket(
  service: ImplementationOf<IAPI>
, ws: WebSocket
): () => void
#+END_SRC
